			...
			((ScrollSmoother.register = function register(e) {
				return (
					L ||
						...
						s() &&
							window.document &&
							((O = window), (N = document), (U = N.documentElement), (K = N.body)),
						D &&
							((q = D.utils.toArray),
							(V = D.utils.clamp),
							(Y = D.parseEase('expo')),
	...
	function ScrollSmoother(t) {
								...
								if (e.offset) {
									if (i) {
										...
										o =
											d +
											s +
											m * ((D.utils.clamp(e.start, e.end, r) - e.start - s) / (n - e.start) - c);
									}
									(o = (function _round(e) {
				...
				if (arguments.length) {
					...
					return (
						t !== n &&
							...
							R && R.observe(n),
							D.set(n, { overflow: 'visible', width: '100%', boxSizing: 'border-box', y: '+=0' }),
		...
		B.forEach(function (e) {
			...
			(this.smooth = function (e) {
				return (
					arguments.length && ((H = ((C = e || 0) && +t.speed) || 1), i.scrubDuration(e)),
			...
			t.effects &&
				this.effects(!0 === t.effects ? '[data-' + k + 'speed], [data-' + k + 'lag]' : t.effects, {
